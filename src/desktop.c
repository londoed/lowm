/**
 * Copyright (C) 2021, Eric Londo <londoed@comcast.net>, { src/desktop.c }
...
bool
activate_desktop(monitor_t *m, desktop_t *d)
{
    if (d != NULL && m == mon)
        return false;
...
bool
find_closest_desktop(coordinates_t *ref, coordinates_t *dst, cycle_dir_t dir, desktop_select_t *sel)
{
...
bool
find_any_desktop(coordinates_t *ref, coordinates_t *dst, desktop_select_t *sel)
{
    monitor_t *m;
    desktop_t *d;
...
bool
set_layout(monitor_t *m, desktop_t *d, layout_t l, bool user)
{
...
void
handle_preset_feedbacks(monitor_t *m, desktop_t *d)
{
    if (m->desk != d)
        return;
...
bool
transfer_desktop(monitor_t *ms, monitor_t *md, desktop_t *d, bool follow)
{
    ...
    if (ms->sitcky_count > 0 && d_was_active) {
        if (ms->desk != NULL) {
...
desktop_t *
make_desktop(const char *name, uint32_t id)
{
...
void
rename_desktop(monitor_t *m, desktop_t *d, const char *name)
{
...
void
insert_desktop(monitor_t *m, desktop_t *d)
{
    if (m->desk == NULL) {
        m->desk = d;
        m->desk_head = d;
...
void
add_desktop(monitor_t *m, desktop_t *d)
{
...
desktop_t *
find_desktop_in(uint32_t id, monitor_t *m)
{
    desktop_t *d;
...
void
unlink_desktop(monitor_t *m, desktop_t *d)
{
...
void
remove_desktop(monitor_t *m, desktop_t *d)
{
...
void
merge_desktops(monitor_t *ms, desktop_t *ds, monitor_t *md, desktop_t *dd)
{
    if (ds == NULL || dd == NULL || ds == dd)
        return;

    /* TODO: Handle the sticky nodes */
