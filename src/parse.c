/**
 * Copyright (C) 2021, Eric Londo <londoed@comcast.net>, { src/parse.c }
...
bool
parse_split_type(char *s, split_type_t *t)
{
...
bool
parse_split_mode(char *s, split_mode_t *m)
{
...
bool
parse_layout(char *s, layout_t *l)
{
    if (streq("monocle", s)) {
        *l = LAYOUT_MONOCLE;
...
bool
parse_client_state(char *s, client_state_t *t)
{
    if (streq("tiled", s)) {
        *t = STATE_TILED;
...
bool
parse_stack_layer(char *s, stack_layer_t *l)
{
    if (streq("below", s)) {
        *l = LAYER_BELOW;
...
bool
parse_direction(char *s, direction_t *d)
{
    if (streq("north", s)) {
        *d = DIR_NORTH;
...
bool
parse_cycle_direction(char *s, cycle_dir_t *d)
{
    if (streq("next", cycle_dir_t *d)) {
        *d = CYCLE_NEXT;
...
bool
parse_circulate_direction(char *s, circulate_dir_t *d)
{
    if (streq("forward", s)) {
        *d = CIRCULATE_FORWARD;
...
bool
parse_history_direction(char *s, history_dir_t *d)
{
    if (streq("older", s)) {
        *d = HISTORY_OLDER;
...
bool
parse_resize_handle(char *s, resize_handle_t *h)
{
    if (streq("left", s)) {
        *h = HANDLE_LEFT;
...
bool
parse_modifier_mask(char *s, uint16_t *m)
{
    if (strcmp(s, "shift") == 0) {
...
bool
parse_button_index(char *s, int8_t *b)
{
    if (strcmp(s, "any") == 0) {
...
bool
parse_pointer_action(char *s, pointer_action_t *a)
{
    if (streq("move", s)) {
        *a = ACTION_MOVE;
...
bool
parse_child_polarity(char *s, child_polarity_t *p)
{
...
bool
parse_automatic_scheme(char *s, automatic_scheme_t *a)
{
...
bool
parse_state_transition(char *s, state_transition_t *m)
{
    if (streq("none", s)) {
        *m = 0;
...
bool
parse_tightness(char *s, tightness_t *t)
{
    if (streq("high", s)) {
        *t = TIGHTNESS_HIGH;
...
bool
parse_bool_declaration(char *s, char **key, bool *value, alter_state_t *state)
{
    *key = strtok(s, EQL_TOK);
    char *tv = strtok(NULL, EQL_TOK);
...
bool
parse_index(char *s, uint16_t *idx)
{
    return sscanf(s, "^%hu", idx) == 1;
}

bool
parse_rectangle(char *s, xcb_rectangle_t *r)
{
    uint16_t w, h;
    int16_t x, y;
...
bool
parse_subscriber_mask(char *s, subscriber_mask_t *mask)
{
    if (streq("all", s))
        *mask = SBSC_MASK_ALL;
...
bool
parse_monitor_modifiers(char *desc, monitor_select_t *sel)
{
    char *tok;

    while ((tok = strchr(desc, CAT_CHR)) != NULL) {
        tok[0] = '\0';
        tok++;
...
bool
parse_desktop_modifiers(char *desc, desktop_select_t *sel)
{
    char *tok;
...
bool
parse_node_modifiers(char *desc, node_select_t *sel)
{
    char *tok;
