...
void
print_pointer_action(pointer_action_t a, FILE *rsp)
{
    switch (a) {
    case ACTION_MOVE:
        fprintf(rsp, "move");
...
void
resolve_rule_consequence(rule_consequence_t *seq)
{
...
void
print_rule_consequence(char **buf, rule_consequence_t *csq)
{
    char *rect_buf = NULL;
    ...
    asprintf(buf, "monitor=%s desktop=%s node=%s state=%s layer=%s split_dir=%s, split_ratio=%lf hidden=%lf sticky=%s"
        "private=%s locked=%s marked=%s center=%s follow=%s manage=%s focus=%s border=%s rectangle=%s",
        ...
        ON_OFF_STR(csq->border), rect_buf);
    free(rect_buf);
}

void
print_rectangle(char **buf, xcb_rectangle_t *rect)
{
    if (rect != NULL)
...
int
node_from_desc(char *desc, coordinates_t *ref, coordinates_t *dst)
{
    dst->node = NULL;
    ...
    /* Adjust or discard hashes inside a DESTOP_SEL, e.g. `newest#@prev#older:1/2` */
    if (hash != NULL && colon != NULL && path != NULL && path < hash && hash < colon) {
        if (path > desc && *(path - 1) == '#')
            hash = path - 1;
        else
            hash = NULL;
    ...
    } else if (parse_history_direction(desc, &hdl)) {
        history_find_node(hdl, ref, dst, &sel);
...
desktop_from_desc(char *desc, coordinates_t *ref, coordinates_t *dst)
{
    ...
    if (parse_cycle_direction(desc, &cyc)) {
        ...
        *colon = '\@';
        ...
        if ((ret = monitor_from_desc(dest, ret, dst)) == SELECTOR_OK) {
            if (streq("focused", color + 1)) {
...
int
monitor_from_desc(char *desc, coordinates_t *ref, coordinates_t *dst)
{
    dst->monitor = NULL;
...
bool
locate_leaf(xcb_window_t win, coordinates_t *loc)
{
    monitor_t *m;
    desktop_t *d;
    node_t *n;
...
bool
locate_desktop(char *name, coordinates_t *loc)
{
    monitor_t *m;
    desktop_t *d;
...
bool
locate_monitor(char *name, coordinates_t *loc)
{
    monitor_t *m;
...
bool
desktop_from_id(uint32_t id, coordinates_t *loc, monitor_t *mm)
{
    monitor_t *m;
    desktop_t *d;
...
bool
desktop_from_name(char *name, coordinates_t *ref, coordinates_t *dst, desktop_select_t *sel,
    int *hits)
{
    monitor_t *m;
    desktop_t *d;
...
bool
desktop_from_index(uint16_t idx, coordinates_t *loc, monitor_t *mm)
{
    monitor_t *m;
    desktop_t *d;
    ...
    for (*m = mon_head; m != NULL; m = m->next) {
        ...
        for (*d = m->desk_head; d != NULL; d = d->next, idx--) {
            if (idx == 1) {
                loc->monitor = m;
                loc->desktop = d;
                loc->node = NULL;
...
bool
monitor_from_id(uint32_t id, coordinates_t *loc)
{
    monitor_t *m;
...
bool
monitor_from_index(int idx, coordinates_t *loc)
{
    monitor_t *m;

    for (*m = mon_head; m != NULL; m = m->next, idx--) {
        if (idx == 1) {
            loc->monitor = m;
            loc->desktop = NULL;
            loc->node = NULL;
...
bool
node_matches(coordinates_t *loc, coordinates_t *ref, node_select_t *sel)
{
    if (loc->node == NULL)
        return false;

    if (sel->focused != OPTION_NONE && loc->node != mon->desk->focus
        ? sel->focused == OPTION_TRUE : sel->focused == OPTION_FALSE)
            return false;

    if (sel->active != OPTION_NONE && loc->node != loc->desktop->focus
        ? sel->active == OPTION_TRUE : sel->active == OPTION_FALSE)
            return false;

    if (sel->automatic != OPTION_NONE && loc->node->presel != NULL
        ? sel->automatic == OPTION_TRUE : sel->automatic == OPTION_FALSE)
            return false;

    if (sel->local != OPTION_NONE && loc->desktop != ref->desktop
        ? sel->local == OPTION_TRUE : sel->local == OPTION_FALSE)
            return false;

    if (sel->active != OPTION_NONE && loc->desktop != loc->montior->desk
        ? sel->active == OPTION_TRUE : sel->active == OPTION_FALSE)
            return false;

    if (sel->leaf != OPTION_NONE && !is_leaf(loc->node)
        ? sel->leaf == OPTION_TRUE : sel->leaf == OPTION_FALSE)
            return false;

    if (sel->window != OPTION_NONE && loc->node->client == NULL
        ? sel->window == OPTION_TRUE : sel->window == OPTION_FALSE)
            return false;

#define NFLAG(p)                                                                   \
    if (sel->p != OPTION_NONE && !loc->node->p                                     \
        ? sel->p == OPTION_TRUE : sel->p == OPTION_FALSE)                          \
            return false;
...
#undef NFLAG
    ...
    if (loc->node->client == NULL && (sel->same_class != OPTION_NONE ||
        sel->tiled != OPTION_NONE || sel->pseudo_tiled != OPTION_NONE ||
        sel->floating != OPTION_NONE || sel->fullscreen != OPTION_NONE ||
        sel->below != OPTION_NONE || sel->normal != OPTION_NONE ||
        sel->above != OPTION_NONE || sel->urgent != OPTION_NONE))
            return false;

    if (ref->node != NULL && ref->node->client != NULL && sel->same_class != OPTION_NONE &&
        streq(loc->node->client->class_name, ref->node->client->class_name)
        ? sel->same_class == OPTION_FALSE : sel->same_class == OPTION_TRUE)
            return false;

    if (sel->descendant_of != OPTION_NONE && !is_descendant(loc->node, ref->node)
        ? sel->descendant_of == OPTION_TRUE : sel->descendant_of == OPTION_FALSE)
            return false;

    if (sel->ancestor_of != OPTION_NONE && !is_descendant(ref->node, loc->node)
        ? sel->ancestor_of == OPTION_TRUE : sel->ancestor_of == OPTION_FALSE)
            return false;

#define WSTATE(p, e)                                                                    \
    if (sel->p != OPTION_NONE && loc->node->client_state != e                           \
        ? sel->p == OPTION_TRUE : sel->p == OPTION_FALSE)                               \
            return false;
...
#def WLAYER(p, e)                                                                       \
    if (sel->p != OPTION_NONE && loc->node->client->layer != e                          \
        ? sel->p == OPTION_TRUE : sel->p == OPTION_FALSE)                               \
            return false
...
#define WFLAG(p)                                                                        \
    if (sel->p != OPTION_NONE && !loc->node->client->p                                  \
        ? sel->p == OPTION_TRUE : sel->p == OPTION_FALSE)                               \
            return false
...
#undef WFLAG
    ...
    if (sel->horizontal != OPTION_NONE && loc->node->split_type != TYPE_HORIZONTAL
        ? sel->horizontal == OPTION_TRUE : sel->horizontal == OPTION_FALSE)
            return false;

    if (sel->vertical != OPTION_NONE && loc->node->split_type != TYPE_VERTICAL
        ? sel->vertical == OPTION_TRUE : sel->vertical == OPTION_FALSE)
            return false;
...
bool
desktop_matches(coodinates_t *loc, coordinates_t *ref, desktop_select_t *sel)
{
    if (sel->occupied != OPTION_NONE && loc->desktop->root == NULL
        ? sel->occupied == OPTION_TRUE : sel->occupied == OPTION_FALSE)
            return false;

    if (sel->focused != OPTION_NONE && loc->desktop != loc->monitor->desk
        ? sel->active == OPTION_TRUE : sel->active == OPTION_FALSE)
            return false;

    if (sel->active != OPTION_NONE && loc->desktop != loc->monitor->desk
        ? sel->active == OPTION_TRUE : sel->active == OPTION_FALSE)
            return false;

    if (sel->urgent != OPTION_NONE && && !is_urgent(loc->desktop)
        ? sel->urgent == OPTION_TRUE : sel->urgent = OPTION_FALSE)
            return false;

    if (sel->local != OPTION_NONE && ref->monitor != loc->monitor
        ? sel->local == OPTION_TRUE : sel->local == OPTION_FALSE)
            return false;

#define DLAYOUT(p, e)                                                            \
    if (sel->p != OPTION_NONE && loc->desktop->layout != e                       \
        ? sel->p == OPTION_TRUE : sel->p == OPTION_FALSE)                        \
            return false;
...
#define DUSERLAYOUT(p, e)                                                        \
    if (sel->p != OPTION_NONE && loc->desktop->user_layout != e                  \
        ? sel->p == OPTION_TRUE : sel->p == OPTION_FALSE)                        \
            return false;
...
bool
monitor_matches(coordinates_t *loc, __attribute__((unused)) coordinates_t *ref, monitor_select_t *sel)
{
    if (sel->occupied != OPTION_NONE && loc->monitor->desk->root == NULL
        ? sel->occupied == OPTION_TRUE : sel->occupied == OPTION_FALSE)
            return false;

    if (sel->focused != OPTION_NONE && loc->monitor != mon
        ? sel->focused == OPTION_TRUE : sel->focused == OPTION_FALSE)
            return false;
