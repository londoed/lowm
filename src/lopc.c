/**
 * Copyright (C) 2021, Eric Londo <londoed@comcast.net>, { src/lopc.c }
...
{
    int sock_fd;
    struct sockadd_un sock_address;
    ...
    for (off = 0; rem = sizeof(msg), n = 0; argc > 0 && rem > 0; off += n, rem -= n, argc--, argv++) {
        n = snprintf(msg + off, rem, "%s%c", *argv, 0);
        msg_len += n;
    }
