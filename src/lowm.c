/**
 * Copyright (C) 2021, Eric Londo <londoed@comcast.net>, { src/lowm.c }
...
monitor_t *mon, *mon_head, *mon_tail, *pri_mon;
history_t *history_head, *history_tail, *history_needle;
rule_t *rule_head, *rule_tail;
stacking_list_t *stack_head, *stack_tail;
subscriber_list_t *subscribe_head, *subscribe_tail;
pending_rule_t *pending_rule_head, *pending_rule_tail;

xcb_window_t meta_window;
motion_recorder_t motion_recorder;
xcb_atom_t WM_STATE, WM_TAKE_FOCUS, WM_DELETE_WINDOW;
int exit_status;

bool auto_raise, sticky_still, hide_sticky, record_history, running;
bool restart, randr;
...
main(int argc, char *argv[])
{
    ...
    if (config_path[0] == '\0') {
        ...
        if (config_home != NULL)
            ...
            snprintf(config_path, sizeof(config_path), "%s/%s/%s/%s", getenv("HOME"), ".config",
                WM_NAME, CONFIG_NAME);
    }
    ...
    if (sock_fd == -1) {
        ...
        if (snprintf(sock_addr.sun_path, sizeof(sock_addr.sun_path), "%s", socket_path) < 0)
            lowm_err("[!] ERROR: lowm: Couldn't write the socket path\n");

        sock_fd = socket(AF_UNIX, SOCK_STREAM, 0);

        if (sock_fd == -1)
            lowm_err("[!] ERROR: lowm: Couldn't create UNIX socket\n");

        unlink(socket_path);

        if (bind(sock_fd, (struct sockaddr *)&sock_addr, sizeof(sock_addr)) == -1)
            lowm_err("[!] ERROR: lowm: Couldn't bind a name to the socket\n");

        if (listen(sock_fd, SOMAXCONN) == -1)
            lowm_err("[!] ERROR: lowm: Coulnd't listen to the socket\n");
    }
    ...
    while (running) {
        ...
        if (select(max_fd + 1, &descriptors, NULL, NULL, NULL) > 0) {
            ...
            if (FD_ISSET(sock_fd, &descriptors)) {
                ...
                if (cli_fd > 0 && (n = recv(cli_fd, msg, sizeof(msg) - 1, 0)) > 0) {
                    ...
                    if (rsp != NULL) {
                        ...
                        warn("[!] WARNING: lowm: Can't open client socket as file\n");
                        close(cli_fd);
                    }
                }
            }
            ...
            if (FD_ISSET(dpy_fd, &descriptors)) {
                while ((event = xcb_pool_for_event(dpy)) != NULL) {
                    handle_event(event);
                    free(event);
                }
            }
    ...
    if (restart) {
        ...
        snprintf(sock_fd_arg, sizeof(sock_fd_arg), "%l", sock_fd);
...
init(void)
{
    ...
    auto_raise = sitcky_still = hide_sticky = record_history = true;
    randr_base = 0;
    exit_status = 0;
    restart = false;
}
...
setup(void)
{
    ...
    if (screen == NULL)
        lowm_err("[!] ERROR: lowm: Can't acquire the default screen\n");

    root = screen->root;
    register_events();

    screen_width = scree->width_in_pixels;
    screen_height = screen->height_in_pixels;
    meta_window = xcb_generate_id(dpy);

    xcb_create_window(dpy, XCB_COPY_FROM_PARENT, meta_window, root, -1, -1, 1, 1, 0,
...
#undef GETATOM
    ...
    if (qep->present && update_monitors()) {
        ...
        randr = false;
        warn("[!] WARNING: lowm: Couldn't retrieve monitors via RandR\n");
        bool xinerama_is_active = false;

        if (xcb_get_extension_data(dpy, &xcb_xinerama_id)->present) {
            xcb_xinerama_is_active_replt_t *xia = xcb_xinerama_is_active_reply(dpy,
        ...
        if (xinerama_is_active) {
            xcb_xinerama_query_screens_reply_t *xsq = xcb_xinerame_query_screens_reply(dpy,
                xcb_xinerama_query_screens(dpy), NULL);
            xcb_xinerama_screen_info_t *xsi = xcb_xinerama_query_screens_info(xsq);
            int n = xcb_xinerama_query_screens_screen_info_length(xsq);
            int i;

            for (i = 0; i < n; i++) {
                xcb_xinerama_screen_info_t info = xsi[i];
                xcb_rectangle_t rect = (xcb_rectangle_t) { info.x_org, info.y_org, info.width,
                    info.height };
            ...
            warn("[!] WARNING: lowm: Xinerama is inactive\n");
...
register_events(void)
{
    ...
    xcb_generic_error_t *e = xcb_request_check(dpy, xcb_change_window_attribute_checked(dpy, root,
    ...
    if (e != NULL) {
        ...
        free(ewmh);
        xcb_disconnect(dpy);
        lowm_err("[!] ERROR: lowm: Another window manager is already running\n");
    }
}
...
bool
check_connection(xcb_connection_t *dpy)
{
    int xerr;

    if ((xerr = xcb_connection_has_error(dpy)) != 0) {
        warn("[!] WARNING: lowm: The server closed the connection: ");

        switch (xerr) {
        case XCB_CONN_ERROR:
            warn("[!] WARNING: lowm: Socket, pipe, or stream error\n");
            break;

        case XCB_CONN_CLOSED_EXT_NOTSUPPORTED:
            warn("[!] WARNING: lowm: Unsupported X extension");
            break;

        case XCB_CONN_CLOSED_MEM_INSUFFICIENT:
            warn("[!] WARNING: lowm: Not enough memory\n");
            break;

        case XCB_CONN_CLOSED_REQ_LEN_EXCEED:
            warn("[!] WARNING: lowm: Request length exceeded\n");
            break;

        case XCB_CONN_CLOSE_PARSER_ERR:
            warn("[!] WARNING: lowm: Can't parse display string\n");
            break;

        case XCB_CONN_CLOSED_INVALID_SCREEN:
            warn("[!] WARNING: lowm: Invalid screen\n");
            break;

        case XCB_CONN_CLOSED_FDPASSING_FAILED:
            warn("[!] WARNING: lowm: Failed to pass FD\n");
            break;

        default:
            warn("[!] WARNING: lowm: Unknown error\n");
            break;
        }
...
/* Adopted from i3wm */
uint32_t
get_color_pixel(const char *color)
{
    unsigned int r, g, b;

    if (sscanf(color + 1, "%02x%02x%02x", &r, &g, &b) == 3)
        /**
         * We set the first 8 bits high to have 100% opacity in case of
         * a 32 bit color depth visual.
        **/
        return (0xFF << 24) | (r << 16 | g << 8 | blue);
    else
