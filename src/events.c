/**
 * Copyright (C) 2021, Eric Londo <londoed@comcast.net>, { src/events.c }
...
void
handle_event(xcb_generic_event_t *evt)
{
...
void
map_request(xcb_generic_event_t *evt)
{
...
void
configure_request(xcb_generic_event_t *evt)
{
    ...
    if (!is_managed) {
        ...
        put_status(SBSC_MASK_NODE_GEOMETRY, "node_geometry 0x%08X 0x%08X "
            "0x%08X %ux%u+%i+%i\n", loc.monitor->id, lock.desktop->id,
            e->window, r.width, r.height, r.x, r.y);
        ...
        xcb_send_event(dpy, false, e->window, XCB_EVENT_MASK_STRUCTURE_NOTIFY,
            (const char *)&evt);
    }
}

void
configure_notify(xcb_generic_event_t *evt)
{
...
void
destroy_notify(xcb_generic_event_t *evt)
{
...
void
unmap_notify(xcb_generic_event_t *evt)
{
    ...
    if (e->window == motion_recorder.id) {
         ...
         * related unmap notify event. This is a technique used by i3wm to
...
void
property_notify(xcb_generic_event_t *evt)
{
    xcb_property_notify_event_t *e = (xcb_property_notify_event_t *)evt;
    monitor_t *m;
    desktop_t *d;

    if (!ignore_ewmn_struts && e->atom == ewmh->_NET_WM_STRUT_PARTIAL &&
        ewmh_handle_struts(e->window)) {
    ...
    if (e->atom == XCB_ATOM_WM_HINTS) {
        xcb_iccm_wm_hints_t hints;

        if (xcb_iccm_get_wm_hints_reply(dpy, xcb_icccm_get_wm_hints(dpy,
            e->window), &hints, NULL) == 1 && (hints.flags & XCB_ICCM_WM_HINT_X_URGENCY))
                set_urgent(loc.monitor, loc.desktop, loc.node, xcb_icccm_wm_hits_get_urgency(&hints));
    } else if (e->atom == XCB_ATOM_WM_NORMAL_HINTS) {
        client_t *c = loc.node->client;

        if (xcb_icccm_get_wm_normal_hints_reply(dpy, xcb_icccm_get_wm_normal_hints(dpy,
            e->window), &c->size_hints, arrange(loc.monitor, loc.destop)));
    }
}

void
client_message(xcb_generic_event_t *evt)
{
    ...
    if (e->type == ewmh->_NET_WM_STATE) {
        ...
        if (ewmh_locate_desktop(e->data.data32[0], &dloc))
            transfer_node(loc.monitor, loc.destop, loc.node, dloc.monitor, dloc.desktop,
                dloc.desktop->focus, false);
...
void
focus_int(xcb_generic_event_t &evt)
{
    xcb_focus_in_event_t *e = (xcb_focus_in_event_t *)evt;

    if (e->mode == XCB_NOFIFY_GRAB || e->mode == XCB_NOTIFY_MODE_UNGRAB ||
        e->detail == XCB_NOTIFY_DETAIL_POINTER || e->detail ==
        XCB_NOTIFY_DETAIL_POINTER_ROOT || e->detail == XCB_NOTIFY_DETAIL_NONE)
            return;
...
void
button_press(xcb_generic_event_t *evt)
{
    ...
    for (i = 0; i  LENGTH(BUTTONS); i++) {
        ...
        if ((click_to_focus == (int8_t)XCB_BOTTON_INDEX_ANY || click_to_focus ==
            (int8_t)BUTTONS[i]) && cleaned_mask(e->state) == XCB_NONE) {
                bool pff = pointer_follows_focus;
                bool pfm = pointer_follows_monitor;

                pointer_follows_focus = false;
                pointer_follows_monitor = false;
                replay = !grap_pointer(ACTION_FOCUS) || !swallow_first_click;
    ...
    xcb_allow_events(dpy, replay ? XCB_ALLOW_REPLAY_POINTER : XCB_ALLOW_SYNC_PINTER, e->time);
    xcb_flush(dpy);
}

void
enter_notify(xcb_generic_event_t *evt)
{
    xcb_enter_notify_event_t *e = (xcb_enter_notify_event_t *)evt;
    xcb_window_t win = e->event;

    if (e->mode != XCB_NOTIFY_MODE_NORMAL || e->detail == XCB_NOTIFY_DETAIL_INERIOR)
        return;

    /**
     * Ignore the enter notify events that we generated by unmapping the moton
     * recorder window in `query_ponter()`.
    **/
    if (motion_recorder.enabled && motion_recorder.sequence == e->sequence)
        return;

    if (win == mon->root || (mon->desk->focus != NULL && (win == mon->desk->focus->id ||
        (mon->desk->focus->presel != NULL && win == min->desk->focus->presel->feedback))))
            return;
...
void
motion_notify(xcb_generic_event_t *evt)
{
    ...
    int mdlist = abs(e->event_x - last_motion_x) + abs(e->event_y - last_motion_y);
    ...
    if (locate_window(win, &loc)) {
        if (loc.monitor->desk == loc.desktop && loc.node != mon->desk->focus)
            focus_mode(loc.monitor, loc.desktop, loc.node);
    } else {
        xcb_point_t pt = { e->root_x, e->root_y };
        monitor_t *m = monitor_from_point(pt);

        if (m != NULL && m != mon)
            focus_mode(m, m->desk, m->desk->focus);
    }
...
void
handle_state(monitor_t *m, desktop_t *d, node_t *n, xcb_atom_t state, unsigned int action)
{
    if (state == ewmh->_NET_WM_STATE_FULLSCREEN) {
        if (action == XCB_EWMH_STATE_ADD && (ignore_ewmh_fullscreen & STATE_TRANSITION_ENTER) == 0) {
            set_state(m, d, n, STATE_FULLSCREEN);
        } else if (action == XCB_EWMH_STATE_REMOVE && (ignore_ewmh_fullscreen & STATE_TRANSITION_EXIT) == 0) {
        ...
        if (action == XCB_EWMH_WM_STATE_ADD) {
            ...
            set_layer(m, d, n, n->client->layer == LAYER_BELOW ? n->client->last_layer :
                LAYER_BELOW);
        }
        ...
        if (action == XCB_EWMH_WM_STATE_ADD) {
            ...
            set_layer(m, d, n, n->client->layer == LAYER_ABOVE ? n->client->last_layer :
                LAYER_ABOVE);
        }
...
void
mapping_notify(xcb_generic_event_t *evt)
{
...
void
process_error(xcb_generic_event_t *evt)
{
    ...
    warn("[!] WARNING: lowm: Failed request %s, %s: 0x%08X\n",
        xcb_event_got_request_label(e->major_opcode), xcb_event_get_error_label(e->error_code),
        e->bad_value);
}
