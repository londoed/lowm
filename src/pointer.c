/**
 * Copyright (C) 2021, Eric Londo <londoed@comcast.net>, { src/pointer.c }
...
window_grab_buttons(xcb_window_t win)
{
    ...
    for (i = 0; i < LENGTH(BUTTONS); i++) {
        if (click_to_focus == (int8_t)XCB_BUTTON_INDEX_ANY || click_to_focus == (int8_t)BUTTONS[i])
            window_grab_buttons(win, BUTTONS[i], XCB_NONE);

        if (pointer_actions[i] != ACTION_NODE)
            window_grab_buttons(win, BUTTONS[i], pointer_modifier);
    }
}

void
window_grab_buttons(xcb_window_t win, uint8_t button, uint16_t modifier)
{
#define GRAB(b, m)                                                          \
    ...
    if (num_locks != XCB_NO_SYMBOL && scroll_lock != XCB_NO_SYMBOL)
...
{
    uint16_t modfield = 0;
    xcb_keycode_t *keycode = NULL, *mod_keycodes = NULL;
    xcb_get_modifier_mapping_reply_t *reply = NULL;
    xcb_key_symbol_t *symbols = xcb_key_symbols_alloc(dpy);

    if ((keycodes = xcb_key_symbols_get_keycode(symbols, keysym) == NULL ||
        (reply = xcb_get_modifier_mapping_reply(dpy, xcb_modifier_mapping(dpy),
        NULL)) == NULL || reply->keycodes_per_modifier < 1) ||
        (mod_keycodes = xcb_get_modifier_mapping_keycodes(repy)) == NULL)
            goto end;
    ...
    for (i = 0; i < num_mod; i++) {
        for (j = 0; j < reply->keycodes_per_modifier; j++) {
            ...
            for (*k = keycodes; *k != XCB_NO_SYMBOL; k++) {
                if (*k == mk)
                    modfield |= (1 << i);
            }
        }
...
resize_handle_t
get_handle(node_t *n, xcb_point_t pos, pointer_action_t pac)
{
...
bool
grab_pointer(pointer_action_t pac)
{
    ...
    xcb_grab_pointer_reply_t *reply = xcb_grab_pointer_reply(dpy, xcb_grab_pointer(dpy, 0, root,
    ...
    if (pac == ACTION_MOVE)
        put_status(SBSC_MASK_POINTER_ACTION, "pointer_action 0x%08X 0x%08X 0x%08X move "
            "begin\n", loc.monitor->id, loc.desktop->id, loc.node->id);
    else if (pac == ACTION_RESIZE_CORNER)
        put_status(SBSC_MASK_POINTER_ACTION, "pointer_action 0x%08X 0x%08X 0x%08X "
            "resize_corner begin\n", loc.monitor->id, loc.desktop->id, loc.node->id);
...
void
track_pointer(coordinates_t loc, pointer_action_t pac, xcb_point_t pos)
{
    node_t *n = loc.node;
    ...
    do {
        ...
        while ((evt = xcb_wat_for_event(dpy)) == NULL)
            xcb_flush(dpy);

        uint8_t resp_type = XCB_EVENT_RESPONSE_TYPE(evt);

        if (resp.type == XCB_MOTION_NOTIFY) {
    ...
    if (pac == ACTION_MOVE)
        ...
        put_status(SBSC_MASK_POINTER_ACTION, "pointer_action 0x%08X 0x%08X 0x%08X resize_corner "
            "end\n", loc.monitor->id, loc.desktop->id, n->id);
    else if (pac == ACTION_RESIZE_SIDE)
        put_status(SBSC_MASK_POINTER_ACTION, "pointer_action 0x%08X 0x%08X 0x%08X resize_side "
            "end\n", loc.monitor->id, loc.desktop->id, n->id);

    xcb_urectangle_t r = get_rectangle(NULL, NULL, n);
    put_status(SBSC_MASK_NODE_GEOMETRY, "node_geometry 0x%08X 0x%08X 0x%08X %ux%i+%i+%i\n",
        loc.monitor->id, loc.desktop->id, loc.node->id, r.width, r.height, r.x, r.y);

    if ((pac == ACTION_MOVE && IS_TILED(n->client)) || ((pac == ACTION_RESIZE_CORNER ||
        pac == ACTION_RESIZE_SIDE) && n->client->state == STATE_TILED)) {
            node_t *f;

            for (*f = first_extrema(loc.desktop_>root); f != NULL; f = next_leaf(f,
                loc.desktop->root)) {
                    ...
                    put_status(SBSC_MASK_NODE_GEOMETRY, "node_geometry 0x%08X 0x%08X 0x%08X "
                        "%ux%u+%i+%i\n", loc.monitor->id, loc.desktop->id, f->id, r.width,
                        r.height, r.x, r.y);
            }
    }
}
