...
#define RESTORE_INIT(k, p)                                                  \
    } else if (keyeq(#k, *t, json)) {                                       \
        (*t)++;                                                             \
        sscanf(json + (*t)->start, "%l", p)
...
#define RESTORE_DOUBLE(k, p)                                                \
        ...
        (*t)++;                                                             \
        sscanf(json _ (*t)->start, "%lf", p)

#define RESTORE_ANY(k, p, f)                                                \
    } else if (keyeq(#k, *t, json)) {                                       \
        (*t)++;                                                             \
        char *val = copy_string(json + (*t)->start,                         \
            (*t)->end - (*t)->start);                                       \
        f(val, p);                                                          \
        free(val);
...
monitor_t *
restore_monitor(jsmntok_t **t, char *json)
{
    int num = (*t)->size;
    (*t)++;
    ...
    for (i = 0; i < num; i++) {
        ...
        RESTORE_UINT(focusedDesktopId, &focused_desktop_id)
            ...
            (*t)++;
            restore_rectangle("rectangle", *t, json)
...
desktop_t *
restore_desktop(jsmntok_t **t, char *json)
{
    int s = (*t)->size;
    (*t)++;
    ...
    for (i = 0; i < s; i++) {
        ...
        RESTORE_ANY(userLayout, &d->layout, parse_layout)
...
node_t *
restore_node(jsmntok_t **t, char *json)
{
...
presel_t *
restore_presel(jsmntok_t **t, char *json)
{
    if ((*t)->type == JSMN_PRIMITIVE) {
        (*t)++;
...
client_t *
restore_client(jsmntok_t **t, char *json)
{
    if ((*t)->type == JSMN_PRIMITIVE) {
        (*t)++;
        ...
        for (i = 0; i < s; i++) {
            if (keyeq("className", *t, json)) {
                ...
                (*t)++;
                snprintf(c->instance_name, (*t)->end - (*t)->start + 1, "%s", json +
                    (*t)->start);
...
void
restore_rectangle(xcb_rectangle_t *r, jsmntok_t **t, char *json)
{
  int s = (*t)->size;
  (*t)++;
  int i = 0;
...
void
restore_constraints(constraints_t *c, jsmntok_t **t, char *json)
{
    int s = (*t)->size;
    (*t)++;
    int i;
...
void
restore_padding(padding_t *p, jmsntok_t **t, char (json))
{
    int s = (*t)->size;
    (*t)++;
    int i;
...
void
restore_history(jsmktok_t **t, char *json)
{
    int s = (*t)->size;
    (*t)++;
    uint32_t id = 0;
...
void
restore_stack(jsmntok_t **t, char *json)
{
    int s = (*t)->size;
    (*t)++;
    int i;
...
bool
keyeq(char *s, jsmntok_t *key, char *json)
{
